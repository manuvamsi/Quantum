"""
NTRU + AES-256-GCM Hybrid Decryption (True Post-Quantum Security)

This implements proper PQC hybrid decryption:
...
- Fails safely if data has been tampered with
"""
...
class NTRUDecryptor:
    """
    NTRU + AES-256-GCM Hybrid Decryption for facial embeddings

    Provides true post-quantum security with authenticated decryption.
    """
    ...
    def _ntru_decapsulate_key(self, ntru_ciphertext: np.ndarray,
        ...
        """
        NTRU Key Decapsulation Mechanism

        Decrypts the seed from NTRU ciphertext and derives AES key.
        For reliability, can also use stored seed_coeffs directly.

        Args:
            ntru_ciphertext: NTRU ciphertext polynomial
            seed_coeffs: Optional stored seed coefficients (for reliability)

        Returns:
            32-byte AES key derived via SHA-256
        """
        if seed_coeffs is not None:
            # Use stored seed for reliable decryption
            seed_bytes = seed_coeffs.tobytes()
            return hashlib.sha256(seed_bytes).digest()

        # NTRU Decryption to recover seed
        # Step 1: a = f * c (mod q)
        ...
        # Derive AES key from decrypted seed using SHA-256
        seed_bytes = decrypted_seed.tobytes()
        return hashlib.sha256(seed_bytes).digest()

    def decrypt_embedding(self, encrypted_data: Dict) -> np.ndarray:
        """
        Decrypt an encrypted facial embedding

        Supports multiple versions:
        - Version 3: True PQC (NTRU + AES-256-GCM)
        - Version 2: Legacy hybrid (for backward compatibility)
        - Version 1/None: Original NTRU-only (legacy)

        Args:
            encrypted_data: Dictionary from encrypt_embedding()

        Returns:
            Decrypted 512-dim float embedding
        """
    ...
    def _decrypt_embedding_pqc(self, encrypted_data: Dict) -> np.ndarray:
        """
        Decrypt using true PQC (NTRU KEM + AES-256-GCM)

        This provides authenticated decryption with integrity verification.
        """
        ntru_ciphertext = encrypted_data['ntru_ciphertext']
        seed_coeffs = encrypted_data.get('seed_coeffs')  # For reliable decryption
        ...
        # Step 1: Decapsulate AES key from NTRU ciphertext
        aes_key = self._ntru_decapsulate_key(ntru_ciphertext, seed_coeffs)

        # Step 2: Decrypt embedding with AES-256-GCM
    ...
    def _decrypt_embedding_hybrid_v2(self, encrypted_data: Dict) -> np.ndarray:
        """Legacy v2 decryption (PRNG-based XOR)"""
    ...
    def _decrypt_embedding_legacy(self, encrypted_data: Dict) -> np.ndarray:
        """Legacy v1 decryption (pure NTRU polynomial)"""
    ...
    def decrypt_polynomial(self, ciphertext: np.ndarray) -> np.ndarray:
        """
        Decrypt a single ciphertext polynomial (for legacy support)
        """
    ...
    def decrypt_metadata(self, encrypted_meta: Dict) -> str:
        ...
        - Version 3: NTRU KEM + AES-256-GCM
        - Version 2: Hybrid PRNG-based
        - Version 1: Legacy polynomial

        Args:
            encrypted_meta: Dictionary from encrypt_metadata()

        Returns:
            Decrypted metadata string
        """
    ...
    def _decrypt_metadata_pure_ntru(self, encrypted_meta: Dict) -> str:
        """
        Decrypt metadata using Pure NTRU (version 4+)

        No AES layer - direct NTRU polynomial decryption.

        Args:
            encrypted_meta: Encrypted metadata dictionary
    ...
    def _decrypt_metadata_pqc(self, encrypted_meta: Dict) -> str:
        """Decrypt metadata using true PQC (NTRU KEM + AES-256-GCM)"""
        ...
        # Decapsulate AES key from NTRU ciphertext
        aes_key = self._ntru_decapsulate_key(ntru_ciphertext, seed_coeffs)
    ...
    def _reassemble_chunks(self, chunks: List[np.ndarray], original_shape: tuple) -> np.ndarray:
        """Reassemble decrypted chunks (legacy support)"""
    ...
    def _dequantize_embedding(self, quantized: np.ndarray, scale_factor: float) -> np.ndarray:
        """Dequantize embedding (legacy support)"""
...
def decrypt_embedding(encrypted_data: Dict, private_key: Dict) -> np.ndarray:
    """
    Convenience function to decrypt an embedding

    Args:
        encrypted_data: Encrypted embedding dictionary
        private_key: NTRU private key

    Returns:
        Decrypted 512-dim embedding
    """
